...
<button bind:this={ref} {...rest} type="button" aria-label="Decrease" aria-controls={numberField.inputId} disabled={numberField.disabled || numberField.readOnly || (numberField.value !== null && numberField.value <= numberField.min)} onclick={composeEventHandlers(onclick, (event) => numberField.stepBy(-1, 'decrement', event))}>
  {@render children?.()}
</button>

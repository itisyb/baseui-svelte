<script lang="ts">
  ...
  let { children, ref = $bindable(null), ...rest }: Props = $props(); const area = getScrollAreaContext();
</script>
{#if area.overflowX && area.overflowY}<div bind:this={ref} {...rest}>{@render children?.()}</div>{/if}
